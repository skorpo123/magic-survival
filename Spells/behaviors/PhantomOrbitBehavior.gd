class_name PhantomOrbitBehavior
...
func _orbit_attack() -> void:
	...
	for _i in range(orbit_count):
		var angle: float = randf() * TAU
		var offset := Vector2(cos(angle), sin(angle)) * orbit_radius
		var hit_pos := pos + offset
		SwarmManager.damage_area(hit_pos, 30.0, dmg)
		EnemyMeshManager.damage_area(hit_pos, 30.0, dmg)
		RunTracker.record_damage(dmg)
		BurstEffectPool.spawn("impact", hit_pos, Color(1.0, 0.3, 0.2))
