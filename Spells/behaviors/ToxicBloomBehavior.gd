class_name ToxicBloomBehavior
extends BaseSpellBehavior

@export var bloom_radius: float = 100.0
@export var bloom_count: int = 3
...
func _bloom() -> void:
	...
	for _i in range(bloom_count):
		var target := SwarmManager.find_closest_pos(pos, 400.0)
		if target == Vector2.ZERO:
			target = EnemyMeshManager.find_closest_pos(pos, 400.0)
		if target == Vector2.ZERO:
			break
		var area := bloom_radius * _spell.get_area_multiplier()
		SwarmManager.damage_area(target, area, dmg)
		EnemyMeshManager.damage_area(target, area, dmg)
		RunTracker.record_damage(dmg)
		if _spell.was_last_crit(): EventBus.crit_landed.emit(dmg, target)
		BurstEffectPool.spawn("explosion", target, Color(0.4, 1.0, 0.2), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
