class_name ArcOverloadBehavior
extends BaseSpellBehavior

@export var overload_radius: float = 100.0
...
func _overload() -> void:
	...
	var area := overload_radius * _spell.get_area_multiplier()
	...
	BurstEffectPool.spawn("explosion", pos, Color(0.9, 0.9, 1.0), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
	JuiceManager.screen_shake(6.0, 0.15)
