...
func _burst() -> void:
	...
	for _i in range(burst_count):
		var target := SwarmManager.find_closest_pos(pos, 500.0)
		if target == Vector2.ZERO:
			target = EnemyMeshManager.find_closest_pos(pos, 500.0)
		if target == Vector2.ZERO:
			break
		var area := burst_radius * _spell.get_area_multiplier()
		...
		BurstEffectPool.spawn("explosion", target, Color(0.35, 0.15, 0.6), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
