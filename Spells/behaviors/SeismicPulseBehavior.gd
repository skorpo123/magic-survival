class_name SeismicPulseBehavior
...
func _pulse() -> void:
	...
	for _i in range(pulse_count):
		var area := pulse_radius * (1.0 + _i * 0.3) * _spell.get_area_multiplier()
		SwarmManager.damage_area(pos, area, dmg * (1.0 - _i * 0.2))
		EnemyMeshManager.damage_area(pos, area, dmg * (1.0 - _i * 0.2))
	RunTracker.record_damage(dmg * pulse_count)

	BurstEffectPool.spawn("explosion", pos, Color(0.5, 0.2, 0.9), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
