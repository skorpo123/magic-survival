class_name FlameWardBehavior
extends BaseSpellBehavior

@export var ward_radius: float = 120.0
@export var tick_count: int = 2
...
func _ward_damage() -> void:
	...
	var area := ward_radius * _spell.get_area_multiplier()
	SwarmManager.damage_area(pos, area, dmg)
	EnemyMeshManager.damage_area(pos, area, dmg)
	RunTracker.record_damage(dmg)

	BurstEffectPool.spawn("explosion", pos, Color(1.0, 0.85, 0.3), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
