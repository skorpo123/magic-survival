class_name PhotonStormBehavior
...
func _storm() -> void:
	...
	for _i in range(bolt_count):
		var target := SwarmManager.find_closest_pos(pos, storm_radius)
		if target == Vector2.ZERO:
			target = EnemyMeshManager.find_closest_pos(pos, storm_radius)
		if target == Vector2.ZERO:
			continue
		var area := 30.0 * _spell.get_area_multiplier()
		SwarmManager.damage_area(target, area, dmg)
		EnemyMeshManager.damage_area(target, area, dmg)
		RunTracker.record_damage(dmg)
		LightningBehavior.spawn_impact(target, Color(1.0, 0.85, 0.3), 1.0)
	JuiceManager.screen_shake(5.0, 0.12)
