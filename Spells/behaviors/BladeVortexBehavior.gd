class_name BladeVortexBehavior
extends BaseSpellBehavior

@export var blade_count: int = 6
@export var vortex_radius: float = 120.0
...
func _vortex_attack() -> void:
	...
	var area := vortex_radius * _spell.get_area_multiplier()
	...
	BurstEffectPool.spawn("explosion", pos, Color(0.8, 0.3, 1.0), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
