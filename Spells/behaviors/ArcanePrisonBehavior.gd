class_name ArcanePrisonBehavior
extends BaseSpellBehavior

@export var prison_radius: float = 80.0
@export var prison_duration: float = 3.0
...
func _create_prison() -> void:
	...
	var area := prison_radius * _spell.get_area_multiplier()
	...
	BurstEffectPool.spawn("explosion", pos, Color(1.0, 0.4, 0.7), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
