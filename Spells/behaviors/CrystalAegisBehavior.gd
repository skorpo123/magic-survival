class_name CrystalAegisBehavior
...
func _aura_damage() -> void:
	...
	var area := aura_radius * _spell.get_area_multiplier()
	SwarmManager.damage_area(pos, area, dmg)
	EnemyMeshManager.damage_area(pos, area, dmg)
	RunTracker.record_damage(dmg)

	BurstEffectPool.spawn("explosion", pos, Color(0.5, 0.8, 1.0), clampf(_spell.get_area_multiplier(), 0.3, 5.0))
