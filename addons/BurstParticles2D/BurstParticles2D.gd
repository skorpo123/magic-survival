...
var arr_p_lifetime : Array[float] = []
var arr_p_elapsed : Array[float] = []
var _burst_timer: SceneTreeTimer = null

var _use_batch: bool = false
var _update_functions: Array[Callable] = []
...
func _new_particle():
	...
	arr_p_lifetime[id] = 0.0
	arr_p_elapsed[id] = 0.0
	return id 
...
func _resize_arrays() -> void:
	...
	arr_p_lifetime.resize(num_particles)
	arr_p_elapsed.resize(num_particles)
...
func _kill_particle(id: int) -> void:
	if id >= arr_dead.size() or arr_dead[id]:
		return
	if id < arr_rid.size() and arr_rid[id] != RID():
...
func _process(delta: float) -> void:
	...
	for p_id in range(num_particles):
		if arr_dead[p_id]:
			continue
		all_dead = false
		arr_p_elapsed[p_id] += delta
		var p_t: float = arr_p_elapsed[p_id] / arr_p_lifetime[p_id]
		if p_t >= 1.0:
			_kill_particle(p_id)
			continue
		arr_t[p_id] = p_t
		t = p_t
		for function in _update_functions:
			function.call(p_t, p_id)
	if all_dead and _use_batch:
		_use_batch = false
		set_process(false)
		_finish()
		if not wait_for_siblings:
			if repeat:
				burst()
...
func burst() -> void:
	...
	if texture:
		...
		_update_functions = _get_update_functions()
		...
		for p_id in range(num_particles):
			...
			arr_p_lifetime[p_id] = maxf(p_lifetime, 0.01)
			arr_p_elapsed[p_id] = 0.0
			var t_start = preprocess_amount if !reverse else 1.0
			arr_p_elapsed[p_id] = t_start * arr_p_lifetime[p_id]

			update_method.call(t_start, p_id, _update_functions)
		...
		if Engine.is_editor_hint():
			...
			for p_id in range(num_particles):
				...
				tween.tween_method(update_method.bind(p_id, _update_functions), t_start, t_end, arr_p_lifetime[p_id])
				tween.tween_callback(_kill_particle.bind(p_id)).set_delay(arr_p_lifetime[p_id])
		else:
			_use_batch = true
			_burst_time = 0.0
			set_process(true)
	...
	if _burst_timer:
		if _burst_timer.timeout.is_connected(_finish):
			_burst_timer.timeout.disconnect(_finish)
		_burst_timer = null
	if not _use_batch:
		_burst_timer = get_tree().create_timer(lifetime, false)
		_burst_timer.timeout.connect(_finish)
		if wait_for_siblings:
			return
		if (repeat or Engine.is_editor_hint()):
			_burst_timer.timeout.connect(burst)
		elif free_when_finished:
			_burst_timer.timeout.connect(queue_free)
